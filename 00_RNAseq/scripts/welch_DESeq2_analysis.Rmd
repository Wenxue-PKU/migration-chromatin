...
```{r}
dat <- read.table('../data/welch_genes', header=TRUE, row.names = 1)
```
...
```{r}
raw_counts <- dat[,c(10:17)]

groups <- data.frame(factor(c("ATRA","ATRA","ATRA","ATRA","TPA","TPA","TPA","TPA")))
rownames(groups) <- colnames(raw_counts)
colnames(groups) <- "condition"
...
dds$condition <- relevel(dds$condition, ref = "ATRA")
...
atravtpa_sig <- atravtpa %>%
  filter(padj<0.05)

write.csv(atravtpa, '../results/atra_v_tpa_all.csv')
write.csv(atravtpa_sig, '../results/atra_v_tpa_sig.csv')
```
